...
class MapsAllPage extends StatefulWidget {
  ...
  MapsAllPage({required this.refreshData});
...
class _MapsAllPageState extends State<MapsAllPage> {
  ...
  late Future<ModelWisata> _kampusFuture;
  ...
  Future<ModelWisata> fetchKampus() async {
    final response = await http.get(Uri.parse('http://192.168.1.17/lat_wisata/listWisata.php'));
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            Set<Marker> markers = snapshot.data!.data.map((kampus) {
              ...
              try {
                ...
                lng = double.parse(kampus.long);
              } catch (e) {
                print('Error parsing lat/lng for kampus: ${kampus.nama}, lat: ${kampus.lat}, long: ${kampus.long}');
